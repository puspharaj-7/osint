...
const Dashboard = () => {
  ...
  const stats = [
    { label: 'Active Cases', value: activeCount, icon: FolderSearch, color: 'text-primary', border: 'border-primary/20', bg: 'bg-primary/5', trend: '+0 today', glow: false },
    { label: 'Total Cases', value: investigations.length, icon: Activity, color: 'text-accent', border: 'border-accent/20', bg: 'bg-accent/5', trend: `${completedCount} completed`, glow: false },
    { label: 'Unread Alerts', value: unreadAlerts, icon: AlertTriangle, color: 'text-warning', border: 'border-warning/20', bg: 'bg-warning/5', trend: unreadAlerts > 0 ? 'Needs attention' : 'All clear', glow: unreadAlerts > 0 },
    { label: 'Avg Risk Score', value: avgRisk, icon: Shield, color: avgRisk >= 70 ? 'text-destructive' : avgRisk >= 40 ? 'text-warning' : 'text-primary', border: avgRisk >= 70 ? 'border-destructive/20' : 'border-warning/20', bg: avgRisk >= 70 ? 'bg-destructive/5' : 'bg-warning/5', suffix: '%', glow: avgRisk >= 70 },
  ];

  // Collect all timeline events across all investigations, sorted newest first
    ...
    <div className="p-4 sm:p-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Real-time investigation oversight and intelligence</p>
        </div>
        <Button
          onClick={() => navigate('/new-investigation')}
          className="bg-primary text-primary-foreground hover:bg-primary/90 font-display tracking-wider gap-2 neon-glow transition-all"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {stats.map((stat, i) => (
          <motion.div
            ...
            className={`glass-panel p-5 border ${stat.border} ${stat.bg} ${stat.glow ? 'neon-glow' : ''} group cursor-default`}
          >
            ...
            {stat.label === 'Avg Risk Score' && (
              <div className="w-full h-1.5 rounded-full bg-secondary mt-2 mb-1 overflow-hidden">
                <motion.div
                  initial={{ width: 0 }}
        ...
        <div className="col-span-1 lg:col-span-2 glass-panel p-6">
          ...
          <div className="space-y-2.5">
            ...
            {investigations.slice(0, 5).map((inv, i) => (
              ...
              >
                {/* Risk-level colored left strip */}
                <div
                  className="w-1 h-10 rounded-full shrink-0"
                  style={{ backgroundColor: riskColor(inv.riskScore) }}
                />
              ...
              {alerts.slice(0, 4).map((alert, i) => (
                ...
                >
                  <div className="flex items-start justify-between gap-2 mb-1">
                    ...
                    {!alert.read && <div className="w-1.5 h-1.5 rounded-full bg-primary mt-1 flex-shrink-0 animate-pulse" />}
                  </div>
